 ...
 *
 * Auto-generated by resource-db-generator@1.0.0 at 2019-01-09T11:09:54.929Z
...
public class GroupServiceTest {
    ...
    private final String PATH = "/kathra/my-group";
    ...
    public void beforeEach(){
        ...
        underTest = new GroupService(dao, sessionService);
    }

    @Test
    public void given_object_when_create_then_works() throws Exception {
        Group object = new Group().path(PATH);
        underTest.create(object);
        Mockito.verify(dao).create(object, CALLER_NAME_MOCKED_SESSION);
    }

    @Test
    public void given_object_existing_path_when_create_then_throws_IllegalArgumentException() throws Exception {
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> {
            Mockito.doReturn(ImmutableList.of(new Group().id(ID).path(PATH))).when(dao).findAll();
            Group object = new Group().path(PATH);
            underTest.create(object);
            Mockito.verify(dao).create(object, CALLER_NAME_MOCKED_SESSION);
        });
        Assertions.assertEquals("Group's path already exists", exception.getMessage());
    }

    @Test
    public void given_object_existing_path_when_update_then_throws_IllegalArgumentException() throws Exception {
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> {
            Mockito.doReturn(ImmutableList.of(new Group().id(ID+"-other").path(PATH))).when(dao).findAll();
            Group object = new Group().id(ID).path(PATH);
    ...
    @Test
    public void given_object_existing_path_when_patch_then_throws_IllegalArgumentException() throws Exception {
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> {
            Mockito.doReturn(ImmutableList.of(new Group().id(ID+"-other").path(PATH))).when(dao).findAll();
            Group object = new Group().id(ID).path(PATH);
    ...
    public void given_objectId_when_findById_then_works() throws Exception {
        Group excepted = new Group().path(PATH);
        Mockito.when(dao.findById("1789")).thenReturn(Optional.of(excepted));
        Optional<Group> returned = underTest.findById("1789");
    ...
    public void given_nothing_when_findAll_then_works() throws Exception {
        List<Group> excepted = ImmutableList.of(new Group().id("10dc3816-1994-4f81-b8d5-f94dbf29dea5"), new Group().id("5sf7sd4qsd46q"));
        Mockito.when(dao.findAll()).thenReturn(excepted);
        List<Group> returned = underTest.findAll();
    ...
    public void given_partialObject_when_patch_then_return_fullObject() throws Exception {
        ...
        ArgumentCaptor<Group> objectCaptor = ArgumentCaptor.forClass(Group.class);
        ...
        Group objectCaptured = objectCaptor.getValue();
    ...
    public Group copy(Group object){
        ModelMapper mapper = new ModelMapper();
        return mapper.map(object, Group.class);
    }
}
