 ...
 *
 * Auto-generated by resource-db-generator@1.0.0 at 2019-01-09T11:09:45.864Z
 * @author julien.boubechtoula
 */
@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
public class ImplementationVersionServiceTest {

    ImplementationVersionService underTest;

    @Mock
    ImplementationVersionDao dao;
    @Mock
    ...
    public void beforeEach(){
        ...
        underTest = new ImplementationVersionService(dao, sessionService, implementationService);
    }
    ...
    public void given_objectId_when_findById_then_works() throws Exception {
        ImplementationVersion excepted = new ImplementationVersion();
        Mockito.when(dao.findById("1789")).thenReturn(Optional.of(excepted));
        Optional<ImplementationVersion> returned = underTest.findById("1789");
    ...
    public void given_nothing_when_findAll_then_works() throws Exception {
        List<ImplementationVersion> excepted = ImmutableList.of(new ImplementationVersion().id("10dc3816-1994-4f81-b8d5-f94dbf29dea5"), new ImplementationVersion().id("5sf7sd4qsd46q"));
        List<String> implementationIdsAuthorized = ImmutableList.of("impl-id-authorized");

        Mockito.when(implementationService.findAllIdsAuthorized()).thenReturn(implementationIdsAuthorized);
        Mockito.when(dao.findAllByImplementationIds(Mockito.eq(implementationIdsAuthorized))).thenReturn(excepted.stream().map(implVersion -> implVersion.getId()).collect(Collectors.toList()));
        Mockito.when(dao.findAll(excepted.stream().map(implVersion -> implVersion.getId()).collect(Collectors.toList()))).thenReturn(excepted);

        List<ImplementationVersion> returned = underTest.findAll();
    ...
    public void given_partialObject_when_patch_then_return_fullObject() throws Exception {
        ...
        ImplementationVersion objectFull = copy(object);
        ...
        ArgumentCaptor<ImplementationVersion> objectCaptor = ArgumentCaptor.forClass(ImplementationVersion.class);
        ...
        ImplementationVersion objectCaptured = objectCaptor.getValue();
    ...
    public ImplementationVersion copy(ImplementationVersion object){
        ModelMapper mapper = new ModelMapper();
        return mapper.map(object, ImplementationVersion.class);
    }
}
